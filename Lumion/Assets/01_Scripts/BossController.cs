...
public class BossController : MonoBehaviour
{
    ...
    private bool _debuffAplicadoPorProximidad;
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        player.AplicarDebuffBoss(multiplicadorMovimientoPlayer, multiplicadorSaltoPlayer);
        IntentarDanio(player);
    }
    ...
    private void OnTriggerExit2D(Collider2D other)
    {
        ...
        player.LimpiarDebuffBoss();
    }

    private void IntentarDanio(PlayerController player)
    {
        if (Time.time < _proximoDanio)
            return;
    ...
    private void DesactivarBoss()
    {
        ...
        if (_player != null)
            _player.LimpiarDebuffBoss();
        _debuffAplicadoPorProximidad = false;
    ...
    private void AplicarEfectoPorProximidad()
    {
        if (_player == null || !_player.gameObject.activeInHierarchy)
            return;

        float distancia = Vector2.Distance(transform.position, _player.transform.position);
        bool cerca = distancia <= Mathf.Max(0.1f, radioEfecto);

        if (cerca)
        {
            _player.AplicarDebuffBoss(multiplicadorMovimientoPlayer, multiplicadorSaltoPlayer);
            IntentarDanio(_player);
            _debuffAplicadoPorProximidad = true;
            return;
        }

        if (_debuffAplicadoPorProximidad)
        {
            _player.LimpiarDebuffBoss();
            _debuffAplicadoPorProximidad = false;
        }
